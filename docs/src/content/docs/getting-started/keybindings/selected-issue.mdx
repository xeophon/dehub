...
Press <kbd>S</kbd> to cycle the currently loaded issues between sorting by latest modified and
sorting by created at, newest to oldest.

## `X` - Toggle Issue Open/Closed

Press <kbd>X</kbd> to close an open issue or reopen a closed issue. When you do, the dashboard uses
the `gh issue close` or `gh issue reopen` command based on the issue's current state.

<Aside type="caution" title="Watch out!">
**Prior to v3.10.0:** When you use this command, the dashboard changes the issue state immediately and
without prompting for confirmation. Only use this command when you're sure you
want to change the issue's state.

**Since v3.10.0:** When you use this command, the dashboard displays a confirmation prompt and
changes the issue state only after you approve the action.
